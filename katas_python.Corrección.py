...
def diferencia_listas(lista1, lista2):
    # Uso zip para emparejar los elementos de ambas listas
...
def filtrar_mascotas(lista_mascotas):
    ...
    # Paso a minúsculas para que no haya problemas si el usuario escribe "mapache" en vez de "Mapache"
    prohibidas_lower = [p.lower() for p in prohibidas]
    return [m for m in lista_mascotas if m.lower() not in prohibidas_lower]
...
class UsuarioBanco:
    ...
    def transferir_dinero(self, destino, cantidad):
        # El dinero sale de self (quien llama) y va a destino
        if not self.cuenta_corriente:
            raise Exception(f"{self.nombre} no tiene cuenta corriente.")
        if cantidad > self.saldo:
            raise Exception(f"Saldo insuficiente en la cuenta de {self.nombre}.")
        self.saldo -= cantidad
        destino.saldo += cantidad
        print(f"Transferencia de {cantidad}€ desde {self.nombre} a {destino.nombre}. Saldo {self.nombre}: {self.saldo}€")
...
bob.transferir_dinero(alicia, 80)             # 3. Transferir 80 de Bob a Alicia
...
def calcular_area(figura, datos):
    if figura == "rectangulo":
        ...
        # datos = (radio,)
        if len(datos) != 1:
            return "Datos incorrectos para círculo"
        radio = datos[0]
